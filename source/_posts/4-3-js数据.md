...
> 表示一个不可变的文件对象，它提供了一系列操作文件的接口。
  ...
  1. 创建方式
    * 构造函数：`new Blob(arr, options)`，二进制对象/字符串的数组、数据类型。
...
### Blob 派生对象
> 继承于底层对象 blob 并进行了扩展，但其内容一样无法修改。
...
#### FileReader 对象 
  ...
  * 异步读取
    * readAsText(file, encoding)：返回纯文本。encoding 指定编码格式（默认 'UTF-8'）。
    * readAsDataURL(file)：返回基于 Base64 编码的 dataURL 字符串（用于读取图像）。
    * readAsBinaryString(file)：返回文件的原始二进制字符串（每个字符表示一字节）。
    * readAsArrayBuffer(file)：返回包含文件内容的 ArrayBuffer 对象。
  * 中止读取：abort() 

...
# 二、插件功能
> 插件是可以被添加到小程序内直接使用的功能组件，它不能独立运行和预览，只能嵌入到其他小程序中使用。开发者可以直接在小程序内使用插件，无需重复开发，为用户提供更丰富的服务。

  * 目前插件开发的开放范围还是企业、媒体或其他组织，暂时不支持个人。
  * 插件对标一个微服务，开发插件必须有 appid，所以一个小程序只能开发一个插件。
  * 开通插件时的插件名称和图片确定后是不允许修改的。
  * 目前没有插件可供搜索的地方，期待微信的插件商城的出现。
  * 插件发布需要通过微信审核(较严格)，插件支持多个线上版本的同时存在。
  * 插件的使用需要申请，插件开发者同意后，使用方可以接入使用插件。
...
> 相同点是都不能独立运行
  ...
  * 插件拥有独立的 API 接口和域名列表，不被小程序本身的域名列表限制。（NPM 依赖库不能进行第三方数据请求）。


## 插件开发
> 官网开通插件功能、开发者工具、新建项目、开发模式为插件

* 项目目录
  * plugin：插件代码目录
  * miniprogram ：调试插件的小程序

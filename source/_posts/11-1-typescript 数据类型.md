...
## TS 类型
  ...
  * 新增类型：Tuple(元组)、Enum(枚举)、Any(任意)、Void(空值)、Never(永不存在的值)。
  ...
  ```ts
    ...
    // Never：用于永远不会结束的函数，是所有类型的子类型
    function error(message: string): never {
      throw new Error(message);
      console.log("never");     // 无法运行到的代码
    }
...
## 函数类型
> 对函数的传入参数和返回值进行约束，注意函数的参数名称不影响函数的匹配。
...
# 六、泛型
> 一种约束宽泛的类型，它的确定类型只在使用时才指定，常用于创建支持不定数据类型的可复用代码。

## 泛型函数
> 实现返回和参数相同类型的值。泛型变量名可以自定义，数量和使用方式对应即可。

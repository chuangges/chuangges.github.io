...
> 指能够读取其他函数内部变量的函数，用来实现私有的方法和变量。
  ...
  * 内存泄露
    ...
    * 解决方案：不要动态绑定事件、不要给动态移除的元素绑定事件而使用事件冒泡在父容器监听事件、动态添加则必须提供 destroy 方法、尽量单例化，少创建 dom，少绑事件。
...
## 加减乘除运算不准确
> 原生 js 运算结果不一定准确，会丢失精度。解决方案是：将浮点数乘以（扩大）10 的 n 次方倍，把浮点数变为整数后再进行相应的运算，最后将得到的结果除以（缩小）10 的 n 次方倍。
...
## 代码执行
  ...
  // 事件循环/异步：script start、async1 start、async2
...
是react 1.6+的新特性，hook是允许从功能组件(function component)“挂钩”React状态和生命周期功能的功能。

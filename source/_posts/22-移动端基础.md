...
description: 开发模式、Viewport 视口、常用样式和方法、移动端优化方案
---
...
# 三、Viewport
> 即浏览器上用来显示网页的部分区域，   手机的宽度浏览器的实际宽度

  <div style="text-indent: 2em">PC 端网站都是以浏览器窗口(设备屏幕) 作为视口进行布局。但是在移动端，viewport 即手机浏览器的实际宽度默认比手机屏幕宽很多，浏览器为了将页面完整显示出来而会将页面缩小，页面内容看起来被压扁，因此引入了 viewport 视口的概念来解决移动端的页面展示问题。</div>

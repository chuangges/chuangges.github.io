...
### viewport 缩放
> 针对不同屏幕分辨率的手机设置等比例缩放。缺点是 PC 端可能会界面变形、布局错乱
...
### 响应式布局
> 通过媒体查询根据不同的屏幕分辨率来进行适配。缺点是 无法覆盖全部分辨率、代码量大、只适合简单的资讯类页面
...
## rem 方案
  ...
  * 实现原理
  ...
  ```js
  // 以设计图 640 分辨率为标准封装 rem.js，网上有优化版 flexible.js 等
  (function (doc, win) {

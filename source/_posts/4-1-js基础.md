...
keywords:
  - js
date: 2019-05-15 00:11:13
description: ECMAScript、DOM、BOM、JS 工具库
---
...
### 基本数据
  * 即原始值：`Number、String、Boolean、undefined、Null、symbol`
...
### 分类
  ...
  * 构造函数：类的标示 (class 是其语法糖)，为初始化的对象添加属性和方法。
...
> 利用事件冒泡原理，将本该元素执行的事件委托给了它的父元素执行  
  ...
  1. 优势：节省内存而提升性能、减少 DOM 操作而提高运行效率。
  2. 适用场景：给多个同类元素绑定相同事件、给动态生成的新元素绑定事件。
  3. 实现代码
  ```js
  // 为三个同级的 input 绑定点击事件
  var box = document.getElementById("box");
...
## jQuery
> 一个 js 库，它把常用方法封装到一个js文件中，需要时直接调用即可。

###  入口函数
  1. js：主流 `window.onload = function(){ }`。
  2. jquery：主流 `$("document").ready(function(){ })`、简化 `$(function(){ })`。
  3. 区别：书写个数不同，前者只能出现一次并且会被覆盖而后者可多个、执行时机不同，前者在文档树加载完成之后还必须 等待图片等所有资源加载完成后才会执行，而后者在文档树加载完成时执行。


### $ 符号
<div style="text-indent: 2em"> jQuery 的别名，本质是一个函数，调用时通过传入不同参数而实现不同功能，可传参数如下：字符串 `$("#id")`、DOM 对象 `$(domObj)`、function `$(function(){ })`。</div>
...
### 基础操作
  ...
  * 操作样式
    * css操作：`css("width")`、`css("width", "20")`、`css({"color": "red", "w": "10px"})`
    * class操作：`addClass()`、`removeClass()`、`toggleClass()`、`hasClass()、is()`

  * 操作 DOM
    * 创建：`var $h3 = $('h3标签')`
    * 插入：`$('父').append/prepend('子')`、`$('子').appendTo('父')`、`$('同胞').before/after('插入同胞')`、`$('插入同胞').insertBefore/After('同胞')`
  ...
  * 操作属性
    ...
    * 方法区别：操作元素的固有属性 `prop`、处理元素的自定义属性 `attr`
  * 操作内容
    * 表单元素：`val(value)`
...
* 发展历程
  ...
  3. on 绑定
    ...
    * 事件解绑：`off()` 解绑元素的所有事件、`off("click", "**")` 解绑元素所有代理的事件但不解绑自身事件。
    * 先解绑再绑定：`$("#btn").off("click").on("click", callback)` 防止事件累加，导致点击一次而事件执行多次。
* 事件触发
  * 直接调用：`$("div").click()`
...
### 编程思想
  ...
  * 多库共存
    * 问题：多个引用库的 $ 符号可能冲突。
    * 解决：`(function($){ })(jQuery)、jQuery(function($){ })、$.noConflict()`。
...
> 为 jQuery 原型添加功能函数来扩展功能。
  ...
  * 关系：`$.fn.pluginName = $.fn.extend(pluginName) = $.prototype.extend(pluginName)`
  ...
  /**
    * @title 插件模板
    * @desp 分号：开头是为了避免别人的代码不规范引起错误，结尾是为了避免压缩出错。
    * @params window、document：快速查找，而且也便于代码压缩，比如 window 可能被压缩为 w。
    * @params undefined：保证原生，防止他人把其定义为别的。
    *
    *
  **/
  ;(function(win, doc, $, undefined){    // 没有第四个参数
    // 写法一
    $.fn.extend({

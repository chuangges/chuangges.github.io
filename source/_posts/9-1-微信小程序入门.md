...
description: 小程序的特点和基础语法、组件化开发、路由功能、疑问汇总
---
...
## 主要特点
  ...
  * 基于微信跨平台
  * 即用即走，不需要安装和卸载
  * 提供原生 APP 体验的服务：这是小程序框架开发的目标。
  * 去中心化：微信不会提供类似小程序商店的地方，需要小程序提供者自己通过二维码，群分享的手段来传播。
  * 页面渲染较快：Native 会预先额外加载一个 WebView，打开页面时直接渲染默认数据然后进行局部更新。页面返回时显示历史 View，退出小程序时不销毁 View 状态 (视图层有多个 webview 线程)。


## 区别 H5

  * __系统权限__：小程序可以获得更多的系统权限，如网络通信状态、数据缓存能力等。
  * __运行环境__：小程序基于浏览器内核重构的内置解析器，而 h5 的宿主环境是浏览器。所以小程序中没有 DOM 和 BOM 的相关 API。
  * __渲染机制__：小程序的逻辑层和渲染层分别在不同的线程中运行 (相互独立)，而 H5 页面的 UI 渲染、Js 脚本执行都在一个单线程 (互斥)。所以 H5 页面中长时间的脚本运行可能会导致页面失去响应。
 

 

* 视图层将 WXML 转化成对应的 JS 对象
...
## 架构原理
  ...
  * 框架由 `View 视图层、App Service 逻辑层` 组成，两者在相互独立的两个线程运行，实现了视图层和逻辑层的分离。
  * 视图层通过 `WKWebView (IOS) / XWeb (Android)` 渲染页面结构，逻辑层通过 `JSCore (IOS) / V8 (Android)` 解析 js 脚本处理业务逻辑。
  * Native 系统层通过 `JSBridge` 向上提供微信的原生功能，通过 `Event、Data` 提供视图层和逻辑层的双向交互 (视图层绑定/捕获事、逻辑层提供数据)。


  * 视图层和逻辑层的数据传输实际上是通过两边提供的 evaluateJavascript 实现的，数据转换为字符串之后需要拼接为 JS 脚本并传递到两边独立环境。但受环境影响而导致数据不是实时到达视图层，setData 是异步执行的

https://www.jianshu.com/p/ca720137818f

https://www.cnblogs.com/yangpeng0517/p/7458703.html

https://blog.csdn.net/fishmai/article/details/90723221


https://blog.csdn.net/lianghaihsia/article/details/84140817

https://www.cnblogs.com/wuhuacong/p/7223785.html



## 虚拟 DOM 机制
> 用 JS 对象模拟DOM树 -> 比较两个DOM树 -> 比较两个DOM树的差异 -> 把差异应用到真正的DOM树上

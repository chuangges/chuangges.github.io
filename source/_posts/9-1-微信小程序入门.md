...
## 技术模块
  ...
  * __View（视图层）__：包括 WXML 和 WXSS 。
  * __App Service（逻辑层）__：使用 JS 来构建业务逻辑。
  * __Native（系统层）__：通过 JS Bridge 向上提供微信的原生能力，通过 Event 和 Data 做双向交互。
...
# 四、插件功能
> 插件是对一组 js 接口、自定义组件或页面的封装，它不能独立运行而必须嵌入在小程序中才能被用户使用，适用于封装自己的功能或服务并提供给第三方小程序进行展示和使用 (个人小程序没有插件功能)。
